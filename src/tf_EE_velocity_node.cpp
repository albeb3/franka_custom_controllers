...
void SetConfiguration(tf::TransformListener& tf_listener){
    ...
    try {
        tf_listener.lookupTransform(arm_id_+"_link0", arm_id_+"_EE", ros::Time(0), start_PandaLink0_T_PandaEE);
    }
...
int main(int argc, char** argv){
    ...
    std::string arm_id;
    if(!private_node.getParam("arm_id", arm_id)){
        ROS_ERROR("Parameter arm_id not set");
        return -1;
    }
    arm_id_ = arm_id;
    std::string child_frame_joystick;
    if(!private_node.getParam("child_frame_joystick", child_frame_joystick)){
        ROS_ERROR("Parameter child_frame_joystick not set");
        return -1;
    }
    double deltat;
    if(!private_node.getParam("deltat", deltat)){
        ROS_ERROR("Parameter deltat not set");
        return -1;
    }
    deltat_ = deltat;
    double limit;
    if(!private_node.getParam("limit", limit)){
    ...
    tfBuffer.canTransform(arm_id_ + "_link0", arm_id_ + "_EE", ros::Time(0), ros::Duration(10.0));
    ...
    ros::Subscriber sub_return_to_mission = node.subscribe<std_msgs::Bool>("my_joint_velocity_controller/return_to_mission/", 10,boost::bind(&Callback_return_to_mission, _1, tf_listener_ptr.get()));
    ros::Subscriber sub_follow_EE = node.subscribe<std_msgs::Bool>("my_joint_velocity_controller/teleop_cmd_received/", 10,boost::bind(&Callback_follow_EE, _1, tf_listener_ptr.get()));
    ros::Rate rate(100.0);
